...
const AnalyticsCard = ({ metric, index }) => {
  const getTrendIcon = (trend) => {
    if (trend === 'up') return FiArrowUp;
    if (trend === 'down') return FiArrowDown;
    return FiMinus;
  };
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`flex items-center space-x-1 text-sm ${getTrendColor(metric.trend)}`}>
          <SafeIcon icon={getTrendIcon(metric.trend)} className="w-3 h-3" />
        ...
        <div className="space-y-2">
          {metric.subMetrics.map((sub, idx) => (
            <div key={idx} className="flex items-center justify-between text-xs">
              ...
              <span className="font-medium text-gray-700 dark:text-gray-300">{sub.value}</span>
            </div>
          ))}
        </div>
      )}
